...
class StackExchangeSource(BaseSource):
 ...
 def collect(self,q:str | tuple[str, ...] | list[str],since: str|None=None):
  out=[]
  for phrase in self.queries(q):
   out.extend(self.parse(self.fetch_json(f"https://api.stackexchange.com/2.3/search/advanced?site=stackoverflow&order=desc&sort=creation&filter=withbody&q={quote_plus(phrase)}&pagesize={self.limit}"),phrase))
  return self.keep_since(out,since)
 def parse(self,data:dict,q:str):
  return [raw_signal(source="stackexchange",family=self.family,external_id=str(d["question_id"]),url=d["link"],query=q,title=d["title"],body=d.get("body") or d["title"],author=d.get("owner",{}).get("display_name"),published_at=datetime.fromtimestamp(d["creation_date"],timezone.utc).isoformat() if d.get("creation_date") else None,lang=detect_language(d.get("body") or d["title"]),cfg=self.cfg) for d in data.get("items",[])[:self.limit]]
